...
echo   3^) Broker-3    - config\broker-3.properties    ^(PLAINTEXT  :9095^)
echo.
...
echo   localhost:9092,localhost:9095
echo.
echo To also run broker-2 ^(:9094^), open another terminal:
echo   cd /d %KAFKA_HOME%
echo   bin\windows\kafka-server-start.bat config\broker-2.properties
echo.
